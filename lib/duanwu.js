...
import { getActiveSeason, getSeasonById } from './seasonal-fish.js';
...
export function isSeasonalFishActive(fish, dateKey = getTodayKey()) {
  if (!fish?.seasonal) return true;
  const seasonId = String(fish.seasonal.eventId || '').trim();
  const season = getSeasonById(seasonId);
  if (!season || season.archived) return false;

  // The season catalog is authoritative. A fish's own dates are a second guard
  // against configuration mistakes, while only one active season can enter the pool.
  const activeSeason = getActiveSeason(dateKey);
  return activeSeason?.id === seasonId &&
    isDateInRange(dateKey, season.startDate, season.endDateExclusive) &&
    isDateInRange(dateKey, fish.seasonal.startDate, fish.seasonal.endDateExclusive);
}
...
export function getDuanwuEasterEggGiftFishName() {
  return String(DUANWU_EVENT_CONFIG.easterEggGift?.fishName || '').trim();
}

export function shouldTriggerDuanwuEasterEggGift(userData, dateKey = getTodayKey()) {
  if (!isDuanwuEventActive(dateKey)) return false;
  const fishName = getDuanwuEasterEggGiftFishName();
  if (!fishName) return false;
  const ownedEggs = Array.isArray(userData?.easterEggCollection) ? userData.easterEggCollection : [];
  if (ownedEggs.includes(fishName)) return false;
  const chance = clampNumber(DUANWU_EVENT_CONFIG.easterEggGift?.chance ?? 0, 0, 1);
  return Math.random() < chance;
}
