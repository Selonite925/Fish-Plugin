...
import { getSeasonById } from './seasonal-fish.js';
...
export function isSeasonalFishActive(fish, dateKey = getTodayKey()) {
  ...
  const season = getSeasonById(fish.seasonal.eventId);
  if (season?.archived) return false;
  return isDateInRange(dateKey, fish.seasonal.startDate, fish.seasonal.endDateExclusive);
}
