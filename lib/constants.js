  ...
  gold_humble: {
    ...
    targetFishEffect: {
      ...
      rewardCoinsByRarity: {
        ...
        epic: 120,
        legendary: 280,
        [EASTER_EGG_RARITY]: 280
      }
    },
    suppressExtraCoinBonuses: true,
    description: '金意很满，竿梢却压得谦虚。可用 #金谦指定 鱼名 锁定目标：钓到目标稀有度时会翻看数个候选，若目标在其中就优先钓起；命中目标给鱼币奖励。代价是目标检索生效时，鱼讯、彩蛋等其它额外鱼币收益不生效，整体稀有度也更克制。'
  }
};
...
export const LOTTERY_GRAND_PRIZE_PLUGINS = {
  gold_humble_rod: {
    id: 'gold_humble_rod',
    name: '金满而谦虚之竿',
    description: '抽奖大奖接口当前挂载的插件奖励。以后替换大奖时，只需要新增插件项并切换 defaultPluginId。',
    reward: {
      type: 'rod',
      id: LOTTERY_ROD_PLUGINS.gold_humble.id,
      value: 2250,
      duplicateCompensationCoins: 2250
    }
  }
};
...
export const LOTTERY_CONFIG = {
  ...
  regularRewards: [
    { type: 'coins', min: 18, max: 22, weight: 0.32, value: 20, title: '小袋鱼币', desc: '回收台下摸出一小袋鱼币' },
    { type: 'coins', min: 32, max: 40, weight: 0.22, value: 36, title: '沉甸鱼币', desc: '袋子不大，但响得很踏实' },
    { type: 'ticket', count: 1, weight: 0.11, value: 58, title: '额外钓鱼券', desc: '今日上限后可再多抛一竿' },
    { type: 'bait', id: 'special_bait', count: 2, weight: 0.12, value: 30, title: '香谷鱼饵 x2', desc: '稳定好用的垫池鱼饵' },
    { type: 'bait', id: 'deep_bait', count: 1, weight: 0.08, value: 27, title: '沉流鱼饵 x1', desc: '给深水口留一点机会' },
    { type: 'bait', id: 'lottery_gilt_bait', count: 1, weight: 0.06, value: 65, title: '鎏金团饵 x1', desc: '抽奖限定鱼饵' },
    { type: 'bait', id: 'lottery_humble_bait', count: 1, weight: 0.06, value: 45, title: '谦光素饵 x1', desc: '抽奖限定鱼饵' },
    { type: 'coins', amount: 120, weight: 0.03, value: 120, title: '闪金鱼币', desc: '一枚挺会发光的大奖前菜' }
  ]
};
...
export const BAIT_CATALOG = {
  ...
  lottery_gilt_bait: {
    ...
    name: '鎏金团饵',
    aliases: ['鎏金团饵', '鎏金饵', '抽奖鎏金饵'],
    price: 0,
    packSize: 1,
    lotteryOnly: true,
    catchRateBonus: 0.105,
    sizeMultiplier: 1.02,
    weightMultiplier: 1.08,
    rarityBias: {
      common: -0.018,
    ...
    description: '抽奖池里偶尔滚出来的金粉团饵，气味不张扬，但会把更像样的鱼口往前推一点。'
  },
  lottery_humble_bait: {
    id: 'lottery_humble_bait',
    name: '谦光素饵',
    aliases: ['谦光素饵', '谦光饵', '抽奖谦光饵'],
    price: 0,
    packSize: 1,
    lotteryOnly: true,
    catchRateBonus: 0.145,
    baitPreserveChance: 0.14,
    rarityBias: {
      common: 0.018,
      uncommon: 0.01,
    ...
    description: '看上去朴素，入水后却很稳，适合把一竿变成一竿，不急着赌大的。'
  }
};
