...
import { normalizeCoinAmount } from './currency.js';
import { ensureReleaseEcho, getReleaseEchoEffect } from './release-echo.js';
import { ensureMapState, getMapFishTemplateByName, getPlayerMaxHealth, normalizeMapId } from './maps.js';
import { getNowTimestamp, getTodayKey } from './time.js';
import { ensureSeasonalCollections } from './seasonal-fish.js';

let fishIdCounter = 0;
const MAX_CAST_HISTORY = 10000;

function getTankUpgradeRequiredPointsForLevel(currentLevel = 0) {
  const normalizedLevel = Math.min(MAX_TANK_LEVEL, Math.max(0, Math.floor(Number(currentLevel || 0))));
  if (normalizedLevel >= MAX_TANK_LEVEL) {
    return {
      targetLevel: MAX_TANK_LEVEL,
      requiredPoints: 0
    };
  }
...
function normalizeEasterEggKeyword(keyword = '') {
...
export function resolveOwnedEasterEggName(userData, keyword) {
  const normalizedKeyword = normalizeEasterEggKeyword(keyword);
  if (!normalizedKeyword) return '';

  const owned = getOwnedEasterEggCollection(userData);
  const indexMatch = normalizedKeyword.match(/^(?:彩蛋|彩蛋鱼)?(\d{1,3})$/u);
  if (indexMatch) {
    const index = Number(indexMatch[1]) - 1;
    return Number.isInteger(index) && index >= 0 ? owned[index] || '' : '';
  }
  if (owned.includes(String(keyword || '').trim())) return String(keyword || '').trim();

  for (const name of owned) {
    const normalizedName = normalizeEasterEggKeyword(name);
    if (normalizedName === normalizedKeyword) return name;

    const effect = EASTER_EGG_EFFECTS[name] || {};
    const aliases = Array.isArray(effect.aliases) ? effect.aliases : [];
    if (aliases.some(alias => normalizeEasterEggKeyword(alias) === normalizedKeyword)) return name;
  }

  const partialMatches = owned.filter(name => {
    const normalizedName = normalizeEasterEggKeyword(name);
    const effect = EASTER_EGG_EFFECTS[name] || {};
    const aliases = Array.isArray(effect.aliases) ? effect.aliases : [];
    return normalizedName.includes(normalizedKeyword) ||
      aliases.some(alias => normalizeEasterEggKeyword(alias).includes(normalizedKeyword));
  });
...
export function scheduleEasterEggSwitch(userData, name, todayKey) {
  const normalizedName = resolveOwnedEasterEggName(userData, name);
  ...
  if (activeName === normalizedName && !getPendingEasterEggName(userData)) {
    return { ok: false, reason: 'already_active', name: normalizedName };
  }
  if (String(userData?.lastEasterEggSwitchDate || '').trim() === String(todayKey || '').trim()) {
    return { ok: false, reason: 'already_scheduled_today', name: normalizedName };
  }
  if (getPendingEasterEggName(userData) === normalizedName) {
    return { ok: false, reason: 'already_pending', name: normalizedName };
  }
...
export function getEasterEggEffects(userData) {
  ...
  const effect = {
    ...
    deepSeaDamageReduction: 0,
    deepSeaHealthCostReduction: 0,
    deepSeaHealthCostBonus: 0,
    deepSeaHealthRecovery: 0,
    deepSeaFishballRateBonus: 0,
    descriptions: []
  };
  ...
  if (config) {
    ...
    effect.deepSeaDamageReduction = Math.max(0, Math.min(0.8, toFiniteNumber(config.deepSeaDamageReduction, 0)));
    effect.deepSeaHealthCostReduction = Math.max(0, Math.min(10, toFiniteNumber(config.deepSeaHealthCostReduction, 0)));
    effect.deepSeaHealthCostBonus = Math.max(0, Math.min(10, toFiniteNumber(config.deepSeaHealthCostBonus, 0)));
    effect.deepSeaHealthRecovery = Math.max(0, Math.min(20, toFiniteNumber(config.deepSeaHealthRecovery, 0)));
    effect.deepSeaFishballRateBonus = Math.max(0, Math.min(0.2, toFiniteNumber(config.deepSeaFishballRateBonus, 0)));
...
export function normalizeUserData(userData) {
  ...
  userData.tankCapacity = Math.min(
    MAX_TANK_CAPACITY,
    Math.max(DEFAULT_TANK_CAPACITY, toFiniteNumber(userData.tankCapacity, DEFAULT_TANK_CAPACITY))
  );
  ...
  if (!Number.isFinite(Number(userData.tankLevel))) {
    userData.tankLevel = Math.min(
      MAX_TANK_LEVEL,
      Math.max(0, Math.floor((userData.tankCapacity - DEFAULT_TANK_CAPACITY) / TANK_UPGRADE_SIZE))
    );
  } else {
    userData.tankLevel = Math.min(MAX_TANK_LEVEL, Math.max(0, Math.floor(Number(userData.tankLevel))));
  }
  userData.tankCapacity = Math.min(
    MAX_TANK_CAPACITY,
    Math.max(userData.tankCapacity, DEFAULT_TANK_CAPACITY + userData.tankLevel * TANK_UPGRADE_SIZE)
  );
  ...
  if (!Number.isFinite(Number(userData.extraDailyLimit))) {
    ...
    userData.extraDailyLimit = Math.max(0, Math.min(MAX_TANK_LEVEL * TANK_UPGRADE_EXTRA_CASTS, Number(userData.extraDailyLimit)));
  }
  // 旧数据里可能只有 tankLevel / tankCapacity，没有写入 extraDailyLimit；这里按鱼缸等级补足永久加钓数。
  userData.extraDailyLimit = Math.min(
    MAX_TANK_LEVEL * TANK_UPGRADE_EXTRA_CASTS,
    Math.max(userData.extraDailyLimit, tankDailyCastBonus)
  );
  if (userData.tankLevel >= MAX_TANK_LEVEL) {
    userData.tankUpgradeProgress = null;
  } else if (userData.tankUpgradeProgress) {
    const targetLevel = Math.max(1, Math.floor(toFiniteNumber(userData.tankUpgradeProgress.targetLevel, userData.tankLevel + 1)));
    if (targetLevel <= userData.tankLevel || targetLevel > MAX_TANK_LEVEL) {
      userData.tankUpgradeProgress = null;
    } else {
  ...
  userData.coins = normalizeCoinAmount(userData.coins);
  ...
  for (const rodId of Object.keys(userData.rodTargets)) {
    ...
    const targetStore = userData.rodTargets[rodId];
    if (!targetStore || typeof targetStore !== 'object' || Array.isArray(targetStore)) {
      delete userData.rodTargets[rodId];
      continue;
    }
    ...
    if (targetStore.byMap && typeof targetStore.byMap === 'object' && !Array.isArray(targetStore.byMap)) {
      const byMap = {};
      for (const [rawMapId, rawTarget] of Object.entries(targetStore.byMap)) {
        const mapKey = String(rawMapId || '').trim().toLowerCase();
        if (!['pond', 'abyss'].includes(mapKey)) continue;
        const mapId = normalizeMapId(mapKey);
        if (!rawTarget || typeof rawTarget !== 'object' || Array.isArray(rawTarget)) continue;
        const type = String(rawTarget.type || 'fish').trim();
        const rarity = String(rawTarget.rarity || '').trim();
        if (!rarity || !['fish', 'rarity'].includes(type)) continue;
        byMap[mapId] = {
          ...rawTarget,
          type,
          rarity,
    ...
    const type = String(targetStore.type || 'fish').trim();
    const rarity = String(targetStore.rarity || '').trim();
    if (!rarity || !['fish', 'rarity'].includes(type)) {
      delete userData.rodTargets[rodId];
      continue;
    }
    userData.rodTargets[rodId] = {
      ...targetStore,
      type,
      rarity,
      // 存量目标没有地图字段时，按早期版本行为归入初始鱼塘。
      mapId: normalizeMapId(targetStore.mapId || 'pond')
    };
  }
  ...
  const normalizeTargetChangeDate = value => {
  ...
  for (const rodId of Object.keys(userData.rodTargetChangeDates)) {
    if (!ROD_CATALOG[rodId]) {
      ...
      const storedDates = userData.rodTargetChangeDates[rodId];
      if (storedDates && typeof storedDates === 'object' && !Array.isArray(storedDates)) {
        userData.rodTargetChangeDates[rodId] = {
          pond: normalizeTargetChangeDate(storedDates.pond),
          abyss: normalizeTargetChangeDate(storedDates.abyss)
        };
      } else {
        const legacyDate = String(storedDates || '').trim();
        // 早期版本把地图日期对象当成字符串保存过；它不代表一次有效调整。
        userData.rodTargetChangeDates[rodId] = {
          pond: normalizeTargetChangeDate(legacyDate),
          abyss: ''
        };
      }
    }
  ...
  if (!Array.isArray(userData.castHistory)) userData.castHistory = [];
  userData.castHistory = userData.castHistory
    .map(item => ({
      timestamp: Math.max(0, toFiniteNumber(item?.timestamp ?? item, 0)),
      usedTicket: Boolean(item?.usedTicket)
    }))
    .filter(item => item.timestamp > 0)
    .slice(-MAX_CAST_HISTORY);
  userData.castHistorySeedDate = String(userData.castHistorySeedDate || '').trim();
  ...
  const storedHealthMax = toFiniteNumber(userData.healthMax, 0);
  userData.healthMax = Math.max(getPlayerMaxHealth(userData, 0), storedHealthMax);
  userData.health = Math.max(0, Math.min(userData.healthMax, Math.floor(toFiniteNumber(userData.health, userData.healthMax))));
  userData.healthDate = String(userData.healthDate || '').trim();
  ensureReleaseEcho(userData);
  ensureMapState(userData);
...
export function addCastHistory(userData, options = {}) {
  if (!Array.isArray(userData.castHistory)) userData.castHistory = [];
  userData.castHistory.push({
    timestamp: Math.max(0, toFiniteNumber(options.timestamp, getNowTimestamp())),
    usedTicket: Boolean(options.usedTicket)
  });
  if (userData.castHistory.length > MAX_CAST_HISTORY) {
    userData.castHistory = userData.castHistory.slice(-MAX_CAST_HISTORY);
  }
}

export function ensureTodayCastHistorySeed(userData, dateKey = '', options = {}) {
  const normalizedOptions = options && typeof options === 'object'
    ? options
    : { timestamp: options };
  const timestamp = Math.max(0, toFiniteNumber(normalizedOptions.timestamp, getNowTimestamp()));
  const dateKeyResolver = typeof normalizedOptions.dateKeyResolver === 'function'
    ? normalizedOptions.dateKeyResolver
    : date => getTodayKey(date);
  const todayCount = Math.max(0, Math.floor(toFiniteNumber(userData?.today?.count, 0)));
  const normalizedDateKey = String(dateKey || '').trim();
  if (!normalizedDateKey || todayCount <= 0 || String(userData?.castHistorySeedDate || '').trim() === normalizedDateKey) return false;
  if (!Array.isArray(userData.castHistory)) userData.castHistory = [];
  const existingTodayCount = userData.castHistory.filter(item => {
    const itemTimestamp = Number(item?.timestamp || 0);
    return Number.isFinite(itemTimestamp) && itemTimestamp > 0 && dateKeyResolver(new Date(itemTimestamp)) === normalizedDateKey;
  }).length;
  const missingCount = Math.max(0, todayCount - existingTodayCount);
  const usedTicketCount = Math.max(0, Math.min(todayCount, Math.floor(toFiniteNumber(userData?.todayExtraUsed, 0))));
  for (let index = 0; index < missingCount; index += 1) {
    userData.castHistory.push({
      timestamp,
      usedTicket: index >= missingCount - usedTicketCount
    });
  }
  userData.castHistory = userData.castHistory.slice(-MAX_CAST_HISTORY);
  userData.castHistorySeedDate = normalizedDateKey;
  return true;
}
...
export function registerCastUsage(config, userData, rod = null, options = {}) {
  ...
  addCastHistory(userData, { usedTicket });
  return {
    registered: true,
    usedTicket,
...
export function getCatchRate(userData, baitBonus = 0, rodBonus = 0, options = {}) {
  const externalEffectMultiplier = Math.max(0, Math.min(1, toFiniteNumber(options.externalEffectMultiplier, 1)));
  let catchRate = BASE_CATCH_RATE + baitBonus + rodBonus + Number(userData?.achievementCatchRateBonus || 0) * externalEffectMultiplier;
  const easterEggEffect = getEasterEggEffects(userData);
  const releaseEchoEffect = getReleaseEchoEffect(userData);
  catchRate += easterEggEffect.catchRateBonus * externalEffectMultiplier;
  catchRate += releaseEchoEffect.catchRateBonus * externalEffectMultiplier;
