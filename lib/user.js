...
function findFishTemplate(fish) {
  ...
  if (!Array.isArray(rarityPool)) return null;
  return rarityPool.find(item => item.name === fish.name) || null;
}
...
export function canFishToday(config, userData, rod = null) {
  ...
  return userData.today.count < normalLimit || Number(userData.tickets || 0) > 0;
}

export function registerCastUsage(config, userData, rod = null) {
  const normalLimit = getDailyLimit(config, userData, rod);
  if (userData.today.count >= normalLimit) {
...
export function getFishingLimitText(config, userData, rod = null) {
  const normalLimit = getDailyLimit(config, userData, rod);
  if (userData.today.count <= normalLimit && Number(userData.tickets || 0) <= 0) {
    return `${userData.today.count}/${normalLimit}`;
  }
  return `${userData.today.count}/${normalLimit} + 券${Number(userData.todayExtraUsed || 0)}竿，余${Number(userData.tickets || 0)}张`;
}

export function getFishingLimitExhaustedText(config, userData, rod = null) {
  const normalLimit = getDailyLimit(config, userData, rod);
  if (userData.today.count > normalLimit) {
    return `基础次数已用完，额外钓鱼券也用完了。（已钓${userData.today.count}次，基础${normalLimit}次，今日用券${Number(userData.todayExtraUsed || 0)}张）`;
  }
  return `今天的钓鱼次数已用完。（${userData.today.count}/${normalLimit}，没有可用钓鱼券）`;
}
