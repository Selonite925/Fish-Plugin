...
export function getEasterEggEffects(userData) {
  ...
  for (const fish of userData?.fishTank || []) {
    ...
    if (config.description) {
      let detail = config.description;
      if (effect.emptyStreakCatchRateBonus > 0 && effect.emptyStreakCatchRateBonusCap > 0) {
        const streakBonus = Math.min(
          effect.emptyStreakCatchRateBonusCap,
          consecutiveEmpty * effect.emptyStreakCatchRateBonus
        );
        detail += `（当前空军加成 +${(streakBonus * 100).toFixed(0)}%）`;
      }
      effect.descriptions.push(`${name}：${detail}`);
    }
  }
