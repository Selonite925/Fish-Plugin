...
export function parseSellTarget(msg) {
  ...
  if (source === 'tank' && !all && !rarity) {
    const sourceRemovedText = text.replace(/鱼缸/g, ' ');
    for (const match of sourceRemovedText.matchAll(/\d{1,3}/g)) {
      const value = Number(match[0]);
      if (Number.isInteger(value) && value > 0) explicitTankIndexes.push(value - 1);
    }
    if (explicitTankIndexes.length === 0) {
  ...
  const uniqueExplicitTankIndexes = [...new Set(explicitTankIndexes)];
  const resolvedCount = source === 'tank' && uniqueExplicitTankIndexes.length ? uniqueExplicitTankIndexes.length : count;
  return {
    source,
    count: resolvedCount,
    rarity,
    all,
    explicitTankIndexes: uniqueExplicitTankIndexes,
    mode: all ? 'all' : rarity ? (trailingNumberMatch ? 'rarity_index' : 'rarity_all') : (source === 'tank' && uniqueExplicitTankIndexes.length ? 'tank_index' : 'count')
  };
}
