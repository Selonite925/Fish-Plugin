...
function isLikelyDeliveredTimeout(result) {
  ...
  return errors.some(error => {
    const message = String(error?.message || error?.wording || error || '');
...
async function buildPanelImageMessage(img) {
  const imageFile = img?.file ?? img?.data?.file ?? img;
  if (!imageFile) return img;

  try {
    const fileUrl = await Bot.fileToUrl(imageFile, { name: 'fish-panel.jpg', time: 10 * 60 * 1000 });
    const normalizedUrl = typeof fileUrl?.toString === 'function' ? fileUrl.toString() : String(fileUrl || '');
    if (normalizedUrl) {
      return {
        type: 'image',
        data: { file: normalizedUrl }
      };
    }
  } catch {}
