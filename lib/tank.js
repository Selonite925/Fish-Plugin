import { EASTER_EGG_RARITY, MAX_TANK_LEVEL, RARITY_PRIORITY, TANK_UPGRADE_EXTRA_CASTS, TANK_UPGRADE_SIZE } from './constants.js';
import { getFishSellValue } from './economy.js';
import { isFishLocked, isSameFish, removeFishFromList } from './user.js';
...
export function addFishToTank(userData, fish, options = {}) {
  ...
  if (fish?.rarity === EASTER_EGG_RARITY) {
    return {
      changed: false,
      added: false,
      replaced: false,
  ...
  const sortedTank = userData.fishTank
    .filter(item => !isFishLocked(userData, item))
    .sort((a, b) => compareFish(a, b));
  if (!sortedTank.length) {
    return {
      changed: false,
      added: false,
      replaced: false,
...
export function getUpgradeCost(costType, currentLevel = 0) {
  const { targetLevel, requiredPoints, atMaxLevel } = getTankUpgradeRequiredPoints(currentLevel);
  if (atMaxLevel) {
    return {
      rarity: costType === 'legendary' ? 'legendary' : 'epic',
      count: 0,
      points: 0,
      atMaxLevel: true
    };
...
export function applyTankUpgrade(userData) {
  if (Number(userData?.tankLevel || 0) >= MAX_TANK_LEVEL) return false;
