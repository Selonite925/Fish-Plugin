export const FISH_COMMAND_RULES = [
  ...
  { reg: '^#地图(?:\\s+.*)?$', fnc: 'handleMapCommand' },
  { reg: '^#(?:前往|进入)(?:深海|深海裂谷|裂谷)$', fnc: 'goToDeepSea' },
  { reg: '^#(?:返回|离开)(?:鱼塘|初始鱼塘|岸边)$', fnc: 'returnToPond' },
  ...
  { reg: '^#炼竿预览(?:\\s*.*)?$', fnc: 'previewLegendaryRod' },
  { reg: '^#炼竿(?!预览)(?:\\s*.*)?$', fnc: 'craftLegendaryRod' },
  { reg: '^#打窝(?:\\s+.*)?$', fnc: 'addBait' },
  { reg: '^#同步鱼缸$', fnc: 'syncAllFishTanks' },
  { reg: '^#修复鱼数据$', fnc: 'repairFishData' },
  { reg: '^#(?:鱼市(?:\\s+.*|(?:购买|回收|售鱼).*)?|售鱼(?:\\s+.*)?)$', fnc: 'handleMarketCommand' },
  ...
  { reg: '^#(?:鱼竿(?!详情|属性)|换竿|换杆)(.*)$', fnc: 'handleRodCommand' },
  { reg: '^#(?:鱼饵(?!详情|属性|自动\\s*续)|换饵(?!\\s*自动\\s*续))(.*)$', fnc: 'handleBaitCommand' },
  { reg: '^#限时鱼讯$', fnc: 'showDailySignal' },
  { reg: '^#钓鱼成就$', fnc: 'showAchievements' },
  { reg: '^#鱼蛋补偿\\s*.*$', fnc: 'compensateFishCoins' },
  { reg: '^#补鱼(?:\\s+.*)?$', fnc: 'compensateFish' },
