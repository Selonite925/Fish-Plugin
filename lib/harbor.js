...
export const HARBOR_BUFF_EXTENSION_MS = 7 * 24 * 60 * 60 * 1000;
...
export function getHarborEffect(worldState, groupId) {
  ...
  return {
    ...level,
    catchRateBonus: active ? level.catchRateBonus : 0,
    signalBonusCoins: active ? level.signalBonusCoins : 0,
    rarityBias: active ? { ...(level.rarityBias || {}) } : {},
    active,
...
export function applyHarborDonation(worldState, groupId, userId, donation = {}) {
  ...
  harbor.buffExpiresAt = Math.min(
    baseExpiry + HARBOR_BUFF_EXTENSION_MS,
...
export function formatHarborBuffRemaining(harbor, now = Date.now()) {
  ...
  if (remainingMs <= 0) return '已过期，捐赠后重新激活';
