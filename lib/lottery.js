...
function isSeasonalActive(seasonal = null, dateKey = getTodayKey()) {
  if (!seasonal) return true;
  const today = String(dateKey || '').trim();
  const startDate = String(seasonal.startDate || '').trim();
  const endDateExclusive = String(seasonal.endDateExclusive || '').trim();
  if (startDate && today < startDate) return false;
  if (endDateExclusive && today >= endDateExclusive) return false;
  return true;
}

function getActiveRegularRewards(dateKey = getTodayKey()) {
  return LOTTERY_CONFIG.regularRewards.filter(reward => isSeasonalActive(reward.seasonal, dateKey));
}
...
export function formatLotteryProbability(rate) {
...
export function getLotteryRewardMetaText(reward) {
...
export function getLotteryRewardPreviewMetaText(reward) {
...
export function getLotteryRewardResultMetaText(reward, item = {}) {
...
export function getLotteryExpectedValue(options = {}) {
  ...
  const regularRewards = getActiveRegularRewards(options.dateKey);
  const grandPassRate = grandAvailable ? Math.max(0, 1 - Number(LOTTERY_CONFIG.grandPrize.rate || 0)) : 1;
  const regularWeightTotal = regularRewards
...
export function getLotteryPoolSummary(options = {}) {
  ...
  return {
    ...
    regularRewards: getActiveRegularRewards(options.dateKey)
  };
}
