...
export function parseMarketCommand(text = '') {
  const match = String(text || '').trim().match(/^#(鱼市|售鱼)(.*)$/);
  if (!match) return null;

  const command = match[1];
  const tail = String(match[2] || '').trim();
  if (command === '售鱼') {
    return { action: 'sell', keyword: tail, shorthand: false };
  }
  if (!tail) {
    return { action: 'show', keyword: '', shorthand: false };
  }

  const actionMatch = tail.match(/^(购买|回收|售鱼)(.*)$/);
  if (actionMatch) {
    const actionMap = { 购买: 'buy', 回收: 'recycle', 售鱼: 'sell' };
    return {
      action: actionMap[actionMatch[1]],
      keyword: String(actionMatch[2] || '').trim(),
      shorthand: false
    };
  }

  // “#鱼市 商品”只作为购买简写，绝不回落到售鱼。
  return { action: 'buy', keyword: tail, shorthand: true };
}
