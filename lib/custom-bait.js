...
function getDisplayCode(seed) {
  return (Number(seed || 17) % 1679616).toString(36).toUpperCase().padStart(4, '0').slice(-4);
}
...
export function generateCustomBaitFromText(text) {
  ...
  const packSize = 3;
  const price = 96;
  const displayCode = getDisplayCode(seed);
  const displayName = `手作鱼饵-${displayCode}`;
  ...
  const bait = {
    ...
    displayName,
    displayCode,
    aliases: [displayName, `手作鱼饵${displayCode}`, `定制鱼饵${displayCode}`, source],
    price,
    packSize,
    catchRateBonus: 0,
    sizeMultiplier: 1,
  ...
  const traitText = `${bait.tags.slice(0, -1).join('，')}${bait.tags.length > 1 ? '，' : ''}${bait.tags[bait.tags.length - 1]}`;
  bait.description = `一份按私人配方拌出的定制鱼饵，闻着像是各路心思都掺了一点：${traitText}。`;
  bait.displayDescription = bait.description;
  delete bait.tags;
  return bait;
}
