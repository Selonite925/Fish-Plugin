...
function buildCustomBaitDescription(rand) {
  return [
    '一份定制鱼饵',
    pickOne(CUSTOM_BAIT_LOOKS, rand),
    pickOne(CUSTOM_BAIT_SCENTS, rand),
    pickOne(CUSTOM_BAIT_TEXTURES, rand),
    pickOne(CUSTOM_BAIT_FINISHES, rand)
  ].join('，') + '。';
}
...
export function generateCustomBaitFromText(text) {
  ...
  bait.description = buildCustomBaitDescription(rand);
  delete bait.tags;
  return bait;
}
